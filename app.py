...
# --- FUNZIONI DI ELABORAZIONE MAPPA E DATI (AGGIORNATE PER PULIZIA TESTO E JITTERING) ---
def process_portfolio_dataframe(df):
    ...
    if lat_col and lon_col:
        ...
        # JITTERING: Dispersione spaziale per evitare sovrapposizioni esatte (es. Sedi Legali)
        np.random.seed(42)
    ...
    # NLP CLEANING: Normalizzazione intelligente della colonna Operatori
    if 'Operator' not in df.columns: 
        df['Operator'] = "Operatore Non Specificato"
    else:
        # Tutto in maiuscolo
        df['Operator'] = df['Operator'].astype(str).str.upper()
        # Rimuove punteggiatura, punti, virgole e simboli strani
...
def check_cbam_category(cn_code): return "Non Soggetto"

@st.cache_data
def load_taxonomy_json(): return set()
@st.cache_data
def load_nace_hierarchy(): return {}
    ...
    if st.button("Estrai Dati da Yahoo Finance"):
        with st.spinner("Scaricando dati dal mercato..."):
            if not ticker: 
                ...
                except Exception as e: st.error("❌ Connessione bloccata da Yahoo.")
...
with t_triage:
    ...
    if status_normativo == "VSME":
        ...
        vsme_qs_E = ["Consumi Energetici suddivisi (rinnovabili vs fossili)?", "Emissioni GHG Scope 1 e 2?", "Registro dei rifiuti?"]
        vsme_qs_S = ["Dati organico per genere/contratto?", "Monitoraggio infortuni?", "Ore di formazione medie?"]
        vsme_qs_G = ["Policy scritta su etica?", "Referente interno sostenibilità?"]
        
        c_v_E, c_v_S, c_v_G = st.tabs(["🌍 Ambiente", "👥 Sociale", "⚖️ Governance"])
        ...
        if st.button("Calcola Readiness VSME", use_container_width=True):
            ...
            with c2:
                ...
                elif avg_score < 4.0: st.warning("🟡 Pronti per il Modulo Base.")
                else: st.success("🟢 Pronti per Modulo PAT o Business Partner.")
...
with t_rischi:
    ...
    with rt_fisico:
        ...
        # --- CARICAMENTO AUTOMATICO SILENZIOSO ---
        ...
        # Expander discreto per cambiare file
        ...
        if not st.session_state.portfolio_df.empty:
            ...
            # Scenario IPCC che altera il rischio (Scala Colore)
            ...
            # Ricalcolo dinamico rischio basato sullo scenario
            np.random.seed(42) 
...
with t_tax:
    ...
    st.info("Funzionalità caricata correttamente nel backend.")
...
with t_down:
    ...
    if st.button("🪄 Genera Report Direzionale (PDF)"):
        st.success("Download pronto.")import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import yfinance as yf
...
def process_portfolio_dataframe(df):
    ...
    if 'Name' not in df.columns: df['Name'] = "Impianto " + df.index.astype(str)
    if 'Operator' not in df.columns: df['Operator'] = "Operatore Non Specificato"
    if 'ID' not in df.columns: df['ID'] = df.index.astype(str)
    if 'Address' not in df.columns: df['Address'] = "Indirizzo non disponibile"
...
with t_triage:
    ...
    if status_normativo == "VSME":
        ...
        dma_questions_dict = {
            "E": {
                "E1 - Cambiamento Climatico": [
                    ("Danni da aumento temperature o eventi estremi su asset fisici?", "F"),
                    ("Impatto finanziario da tasse o regolamentazioni green?", "F"),
                    ("Impatto delle attività aziendali sulle emissioni globali GHG?", "I")
                ],
                "E2 - Inquinamento": [
                    ("Impatti derivanti dal rilascio di sostanze inquinanti?", "I"),
                    ("Dipendenza da sostanze chimiche pericolose soggette a restrizioni?", "F"),
                    ("Rilevanza inquinamento acustico o luminoso prodotto?", "I")
                ],
                "E3 - Risorse Idriche e Marine": [
                    ("Vulnerabilità operativa per siti in aree ad alto stress idrico?", "F"),
                    ("Influenza scarichi idrici su falde o corpi idrici locali?", "I"),
                    ("Impatto/Dipendenza dalle risorse marine?", "I")
                ],
                "E4 - Biodiversità ed Ecosistemi": [
                    ("Attività operative vicine ad aree protette o alta biodiversità?", "I"),
                    ("Rischio deforestazione tramite la supply chain?", "I"),
                    ("Dipendenza dell'azienda dai 'servizi ecosistemici'?", "F")
                ],
                "E5 - Economia Circolare": [
                    ("Rischi legati alla scarsità di materie prime vergini?", "F"),
                    ("Impatto ambientale derivante dalla gestione dei rifiuti?", "I"),
                    ("Capacità dei prodotti di essere riutilizzati, riparati o riciclati?", "I")
                ]
            },
            "S": {
                "S1 - Forza Lavoro Propria": [
                    ("Impatto delle politiche su salute e sicurezza dipendenti?", "I"),
                    ("Rischio discriminazione (genere, età) in selezione/promozione?", "I"),
                    ("Congruità dei salari rispetto al costo della vita (living wage)?", "I")
                ],
                "S2 - Lavoratori Catena Valore": [
                    ("Rischio di lavoro forzato o minorile presso i fornitori?", "I"),
                    ("Dipendenza da fornitori in paesi con scarse tutele diritti umani?", "F"),
                    ("Canali whistleblowing accessibili a lavoratori supply chain?", "I")
                ],
                "S3 - Comunità Interessate": [
                    ("Impatto (rumore, traffico) siti aziendali sulle popolazioni locali?", "I"),
                    ("Influenza delle attività sui diritti delle popolazioni indigene?", "I"),
                    ("Processo gestione lamentele provenienti dalle comunità?", "I")
                ],
                "S4 - Consumatori ed Utenti": [
                    ("Rischio impatti su salute/sicurezza derivanti dall'uso prodotti?", "I"),
                    ("Rischi legati alla privacy e protezione dati dei clienti?", "F"),
                    ("Impatto socio-economico del marketing su consumatori vulnerabili?", "I")
                ]
            },
            "G": {
                "G1 - Condotta negli Affari": [
                    ("Analisi rischi di corruzione attiva e passiva?", "F"),
                    ("Trasparenza e etica nei processi di lobbying aziendale?", "I"),
                    ("Livello protezione garantito ai whistleblower interni?", "I")
                ]
            }
        }
                ...
                for topic, questions in pillar_dict.items():
                    with st.expander(f"Valuta Tema: {topic}"):
                        ...
                        for idx, (q_text, q_type) in enumerate(questions):
                            ...
                            ans = st.selectbox("Maturità Analisi:", SCALE_OPTIONS, key=f"dma_{pillar_code}_{topic}_{idx}", label_visibility="collapsed")
                            val_num = SCALE_VALUES[ans]
                            if q_type == 'I': imp_sc.append(val_num)
                            else: fin_sc.append(val_num)
        ...
        with t_dma_all:
            ...
            if dma_data:
                df_dma = pd.DataFrame(dma_data)
                fig_dma = px.scatter(df_dma, x="Finanza", y="Impatto", color="Pilastro", color_discrete_map={'E': '#00B050', 'S': '#00B0F0', 'G': '#0070C0'}, size="Dim", hover_name="Tema", text="Tema", range_x=[-0.5, 5.5], range_y=[-0.5, 5.5])
                fig_dma.add_hline(y=2.45, line_dash="dash", line_color="red")
                fig_dma.add_vline(x=2.45, line_dash="dash", line_color="red")
                fig_dma.update_traces(textposition='top center', textfont_size=10)
                ...
                if temi_mat: st.success(f"📌 **Temi Obbligatori ({len(temi_mat)}/10):** " + ", ".join(temi_mat))

# =====================================================================
# TAB 2: ANALISI RISCHI E MAPPATURA ASSET
# =====================================================================
with t_rischi:
    rt_fisico, rt_transizione, rt_credito = st.tabs(["🛰️ Simulazione Geospaziale & Mappa", "🔄 Transizione (GHG)", "💰 Stress Test (NGFS)"])
    ...
    with rt_fisico:
        ...
        with st.expander("🔄 Carica un portfolio diverso (Upload / GitHub)"):
            ...
            with col_m2:
                github_url = st.text_input("Oppure incolla URL GitHub Raw (CSV)", value="")
                use_github = st.checkbox("Usa link GitHub")

            if st.button("🗺️ Genera Mappa da nuovo file"):
    ...
    with rt_transizione:
        ...
        EMISSION_FACTORS = {
            ...
            "Scope 1 - Gasolio": {"scope": "scope1", "unita": {"Litri (L)": 2.68}},
            "Scope 2 - Elettricità (Mix Italia)": {"scope": "scope2", "unita": {"MWh": 259.0}},
            "Scope 2 - Elettricità (100% Rinnovabile GO)": {"scope": "scope2", "unita": {"MWh": 0.0}},
            "Scope 3 - Trasporto Merci": {"scope": "scope3", "unita": {"Tonnellate-km": 0.11}}
        }
